...
ggplot(data = combined, mapping = aes(x = Group, y = as.numeric(Percent), fill = `Underlying Condition`)) +
  ...
  theme(legend.key.size = unit(0.2, "cm"), legend.position = "right") +
  guides(fill = guide_legend(nrow = 22, byrow = TRUE)) + 
  ggtitle("Frequency of Underlying Medical Conditions in Adults Hospitalized with COVID-19") + ylab("Percent") +
